...
import com.meshcentral.agent.control.MouseControl
import com.meshcentral.agent.service.ConnectionService
...
class MainActivity : AppCompatActivity() {
    ...
    private var serviceConnection = "com.meshcentral.agent.service.ConnectionService"
    ...
    private fun isAccessibilityServiceEnabled(): Boolean {
        if (ACCESSIBILITY_SERVICE_NAME == null) {
            return true;
        }
        ...
        if (accessibilityEnabled == 1) {
            ...
            if (settingValue != null) {
                ...
                while (mStringColonSplitter.hasNext()) {
                    ...
                    if (accessibilityService.contains(ACCESSIBILITY_SERVICE_NAME)
                        && accessibilityService.contains(packageName)
                    ) {
                        return true;
                    }
                }
    ...
    private fun gotoAccessibilitySetting(dialog: DialogInterface) {
        val intent = Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS);
        startActivity(intent);
        dialog.cancel();
    }

    private fun showAlertAccessibilityService() {
        val builder1 = AlertDialog.Builder(this)
        builder1.setMessage(
            "Mesh Central Agent Android needs to activate mouse control accessibility service!")
        builder1.setCancelable(true)
        builder1.setPositiveButton("Go to Setting") { dialog, _ -> gotoAccessibilitySetting(dialog) }
    ...
    private fun gotoKeyboardSetting(dialog: DialogInterface) {
        val intent = Intent(Settings.ACTION_INPUT_METHOD_SETTINGS)
        startActivity(intent)
        dialog.cancel()
    }

    private fun showAlertCustomKeyboard() {
        val builder1 = AlertDialog.Builder(this)
        builder1.setMessage(
            "Mesh Central Agent Android needs to use a custom keyboard to perform remote input controlling")
        builder1.setCancelable(true)
        builder1.setPositiveButton("Go to Setting") { dialog, _ -> gotoKeyboardSetting(dialog) }
        builder1.setNegativeButton("Cancel") { dialog, _ -> dialog.cancel() }
        val alert11 = builder1.create();
        alert11.show();
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!isAccessibilityServiceEnabled()) {
            showAlertAccessibilityService()
        }
        if (!isCustomKeyboardEnabled()) {
            showAlertCustomKeyboard()
        }
    ...
    private fun checkStartIntent() {
        val startService: String? = intent.getStringExtra("startService")
        if (startService.equals(SCREEN_CAPTURE_SERVICE)) {
            startProjection();
        }
    }
    ...
    private val startActivityFloatingPermission : ActivityResultLauncher<Intent> = registerForActivityResult(
...
//        println("onActivityResult, requestCode: $requestCode, resultCode: $resultCode, data: ${data.toString()}")
    ...
    fun startProjection() {
        if ((g_ScreenCaptureService != null) || (meshAgent == null) || (meshAgent!!.state != 3)) {
            return;
        }

        if (g_mainActivity == null) {
            ConnectionService.service.startApp()
            return;
        }
        ...
        // Move action start MediaProjectionManager activity from Main Activity to Screen Capture Activity
        val screenCaptureIntent = Intent("com.meshcentral.agent.ScreenCaptureActivity");
...
//        startActivityForResult(mProjectionManager.createScreenCaptureIntent(), MainActivity.Companion.REQUEST_CODE)
    ...
    private fun startConnectionService() {
        ...
        for (service in manager.getRunningServices(Integer.MAX_VALUE) ) {
            if (service.service.className.equals(serviceConnection)) {
                return;
            }
        }
        val serIntent = Intent(applicationContext, ConnectionService::class.java)
        startService(serIntent)
        ConnectionService.Companion.mainActivity = this
    }

    companion object {
        const val REQUEST_CODE = 100
        const val SCREEN_CAPTURE_SERVICE = "ScreenCaptureService"
        val ACCESSIBILITY_SERVICE_NAME: String? = MouseControl::class.java.name
    }
}