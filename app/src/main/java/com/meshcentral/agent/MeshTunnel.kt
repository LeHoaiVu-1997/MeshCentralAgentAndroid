...
import com.meshcentral.agent.control.ActionControl
...
//        println("Tunnel-onMessage: $text")
    ...
    private fun processBinaryDesktopCmd(cmd : Int, cmdsize: Int, msg: ByteString) {
        when (cmd) {
            1 -> { // Legacy key input
                ActionControl.getInstance().dispatchKeyboardData(msg, true);
            }
            2 -> { // Mouse input
                ActionControl.getInstance().dispatchTouchData(msg, cmdsize);
            }
            ...
            85 -> { // Unicode key input
                ActionControl.getInstance().dispatchKeyboardData(msg, false);
            }
            87 -> { // Input Lock
                // Nop
            }
